...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.pretrained:
        ...
        model = models.resnet_binary.__dict__[args.arch](pretrained=True, num_classes=num_classes)
    else:
        print("=> creating model '{}'".format(args.arch))
        model = models.resnet_binary.__dict__[args.arch](num_classes=num_classes)
    ...
    # Freeze main network
    models.switchable_ops.interpolate_SBN_params(model)
...
def train(train_loader, model, criterion, optimizer, epoch, args, convs, bns):
    ...
    top5 = [None] * 2
    progress = [None] * 2
    for i, bitA in enumerate((bitA_array[0], bitA_array[-1])):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        for j, bitA in enumerate((bitA_array[0], bitA_array[-1])):
            # Set active switch
...
def configure_model(convs, bns, bitA):
    bitA = random.randint(bitA_array[0], bitA_array[-1]) if bitA == 'r' else bitA
