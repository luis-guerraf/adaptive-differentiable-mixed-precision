...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model = models.resnet_binary_adaptive.__dict__[args.arch](num_classes=num_classes, switches=len(bitA_array))
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            if 'checkpoint_adaptive' in args.resume:
    ...
    if args.evaluate:
        validate(val_loader, model, criterion, criterion_policy, 0, args)
        return
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(train_loader, model, criterion, criterion_policy, optimizer, epoch, args)

        # evaluate on validation set
        acc1 = validate(val_loader, model, criterion, criterion_policy, epoch, args)
...
def train(train_loader, model, criterion, criterion_policy, optimizer, epoch, args):
    batch_time = AverageMeter('Time', ':6.3f')
    losses = AverageMeter('Loss', ':.2e')
    losses_policy = AverageMeter('Loss', ':.2e')
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        losses_policy.update(loss_policy.item(), images.size(0))
...
def validate(val_loader, model, criterion, criterion_policy, epoch, args):
    batch_time = AverageMeter('Time', ':6.3f')
    losses = AverageMeter('Loss', ':.2e')
    losses_policy = AverageMeter('Loss', ':.2e')
    top1 = AverageMeter('Acc@1', ':6.2f')
...
def get_correct(output, target):
    """Returns boolean mask of correct outputs"""
    with torch.no_grad():
