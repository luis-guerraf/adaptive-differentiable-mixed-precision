...
def interpolate_SBN_params(model):
    for subm in model.modules():
        if (isinstance(subm, SwitchableBatchNorm2d)):
            for i, bn in enumerate(subm.bn):
                if i != 0 and i != (len(subm.bn)-1):
                    bn.weight.data.copy_(subm.bn[0].weight.data * 0.5 +
                                         subm.bn[-1].weight.data * 0.5)
                    bn.bias.data.copy_(subm.bn[0].bias.data * 0.5 +
                                       subm.bn[-1].bias.data * 0.5)
                    bn.running_mean.data.copy_(subm.bn[0].running_mean.data * 0.5 +
                                               subm.bn[-1].running_mean.data * 0.5)
                    bn.running_var.data.copy_(subm.bn[0].running_var.data * 0.5 +
                                              subm.bn[-1].running_var.data * 0.5)
                    bn.num_batches_tracked.data.copy_(subm.bn[0].num_batches_tracked.data * 0.5 +
                                                      subm.bn[-1].num_batches_tracked.data * 0.5)
